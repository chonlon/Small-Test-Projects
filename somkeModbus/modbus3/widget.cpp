...
Widget::~Widget()
{
    ...
    //emit signal_modbus_end();
    if (m_pModbus) {
        m_pModbus->thread()->quit();
        m_pModbus->thread()->wait();
        //m_pModbus->deleteLater();
        m_pModbus = nullptr;
    }
}
...
void Widget::on_modbus_init_clicked()
{
    ...
    if (!m_pModbus) {
        ...
        connect(this, &Widget::signal_modbus_start, 
            m_pModbus, &CModbus::modbus_start);
        connect(this, &Widget::signal_modbus_end, 
            m_pModbus, &CModbus::modbus_end);
        ...
        connect(m_pModbus, &CModbus::signal_data_recv, this, &Widget::onReadResult);
    }
