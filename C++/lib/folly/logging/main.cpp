...
FOLLY_INIT_LOGGING_CONFIG(
    ".=INFO:y,folly=INFO:y; default:async=true,sync_level=WARNING;y=file:path=./log/examplelog.log");

int main(int argc, char* argv[]) {

    // Using log macros before calling folly::initLogging() will use the default
    // log settings defined by folly::initializeLoggerDB().  The default
    // behavior is to log INFO+ messages to stderr.
    XLOG(DBG)
        << "log messages less than INFO will be ignored before initLogging";
    ...
    XLOGF(
        DBG1, "XLOGF supports {}-style formatting: {:.3f}", "python", 1.0 / 3);
